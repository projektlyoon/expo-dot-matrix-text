import type { DotMatrixGlyph } from './types';

const EMPTY: DotMatrixGlyph = ['00000', '00000', '00000', '00000', '00000'];

const CUSTOM_GLYPHS: Record<string, DotMatrixGlyph> = {};

export const GLYPHS: Record<string, DotMatrixGlyph> = {
  'A': ['01110', '10001', '11111', '10001', '10001'],
  'B': ['11110', '10001', '11110', '10001', '11110'],
  'C': ['01111', '10000', '10000', '10000', '01111'],
  'D': ['11110', '10001', '10001', '10001', '11110'],
  'E': ['11111', '10000', '11110', '10000', '11111'],
  'F': ['11111', '10000', '11110', '10000', '10000'],
  'G': ['01111', '10000', '10011', '10001', '01111'],
  'H': ['10001', '10001', '11111', '10001', '10001'],
  'I': ['11111', '00100', '00100', '00100', '11111'],
  'J': ['00111', '00010', '00010', '10010', '01100'],
  'K': ['10001', '10010', '11100', '10010', '10001'],
  'L': ['10000', '10000', '10000', '10000', '11111'],
  'M': ['10001', '11011', '10101', '10001', '10001'],
  'N': ['10001', '11001', '10101', '10011', '10001'],
  'O': ['01110', '10001', '10001', '10001', '01110'],
  'P': ['11110', '10001', '11110', '10000', '10000'],
  'Q': ['01110', '10001', '10101', '10010', '01101'],
  'R': ['11110', '10001', '11110', '10010', '10001'],
  'S': ['01111', '10000', '01110', '00001', '11110'],
  'T': ['11111', '00100', '00100', '00100', '00100'],
  'U': ['10001', '10001', '10001', '10001', '01110'],
  'V': ['10001', '10001', '10001', '01010', '00100'],
  'W': ['10001', '10001', '10101', '11011', '10001'],
  'X': ['10001', '01010', '00100', '01010', '10001'],
  'Y': ['10001', '01010', '00100', '00100', '00100'],
  'Z': ['11111', '00010', '00100', '01000', '11111'],
  '0': ['01110', '10011', '10101', '11001', '01110'],
  '1': ['00100', '01100', '00100', '00100', '01110'],
  '2': ['01110', '10001', '00010', '00100', '11111'],
  '3': ['11110', '00001', '01110', '00001', '11110'],
  '4': ['10010', '10010', '11111', '00010', '00010'],
  '5': ['11111', '10000', '11110', '00001', '11110'],
  '6': ['01111', '10000', '11110', '10001', '01110'],
  '7': ['11111', '00010', '00100', '01000', '01000'],
  '8': ['01110', '10001', '01110', '10001', '01110'],
  '9': ['01110', '10001', '01111', '00001', '11110'],
  ' ': EMPTY,
  '.': ['00000', '00000', '00000', '01100', '01100'],
  ',': ['00000', '00000', '00000', '01100', '01000'],
  ':': ['00000', '01100', '00000', '01100', '00000'],
  ';': ['00000', '01100', '00000', '01100', '01000'],
  '!': ['00100', '00100', '00100', '00000', '00100'],
  '?': ['01110', '10001', '00010', '00000', '00010'],
  "'": ['00100', '00100', '00000', '00000', '00000'],
  '"': ['01010', '01010', '00000', '00000', '00000'],
  '-': ['00000', '00000', '11111', '00000', '00000'],
  '_': ['00000', '00000', '00000', '00000', '11111'],
  '+': ['00000', '00100', '11111', '00100', '00000'],
  '=': ['00000', '11111', '00000', '11111', '00000'],
  '/': ['00001', '00010', '00100', '01000', '10000'],
  '\\': ['10000', '01000', '00100', '00010', '00001'],
  '|': ['00100', '00100', '00100', '00100', '00100'],
  '(': ['00010', '00100', '00100', '00100', '00010'],
  ')': ['01000', '00100', '00100', '00100', '01000'],
  '[': ['01110', '01000', '01000', '01000', '01110'],
  ']': ['01110', '00010', '00010', '00010', '01110'],
  '{': ['00110', '00100', '01100', '00100', '00110'],
  '}': ['01100', '00100', '00110', '00100', '01100'],
  '<': ['00010', '00100', '01000', '00100', '00010'],
  '>': ['01000', '00100', '00010', '00100', '01000'],
  '@': ['01110', '10001', '10111', '10100', '01111'],
  '#': ['01010', '11111', '01010', '11111', '01010'],
  '$': ['01111', '10100', '01110', '00101', '11110'],
  '%': ['11001', '11010', '00100', '01011', '10011'],
  '&': ['01100', '10010', '01100', '10010', '01101'],
  '*': ['10101', '01110', '11111', '01110', '10101'],
  '♥': ['01010', '11111', '11111', '01110', '00100'],
  '★': ['00100', '10101', '01110', '10101', '00100'],
  '•': ['00000', '00100', '01110', '00100', '00000'],
  '↑': ['00100', '01110', '10101', '00100', '00100'],
  '↓': ['00100', '00100', '10101', '01110', '00100'],
  '←': ['00100', '01000', '11111', '01000', '00100'],
  '→': ['00100', '00010', '11111', '00010', '00100'],
};

const FALLBACK_GLYPH = GLYPHS['?'] as DotMatrixGlyph;

function normalizeChar(char: string): string {
  return char.length === 1 ? char.toUpperCase() : char.charAt(0).toUpperCase();
}

export function getGlyph(char: string): DotMatrixGlyph {
  const normalized = normalizeChar(char);

  return CUSTOM_GLYPHS[normalized] ?? GLYPHS[normalized] ?? FALLBACK_GLYPH;
}

export function registerGlyphs(custom: Record<string, DotMatrixGlyph>): void {
  Object.entries(custom).forEach(([char, glyph]) => {
    CUSTOM_GLYPHS[normalizeChar(char)] = glyph;
  });
}
